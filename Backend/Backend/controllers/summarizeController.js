        ...
        for (const txtObj of page.Texts) {
          ...
          for (const frag of fragments) {
            try {
              ...
              trackBackendEvent("PDF_TEXT_DECODE_FAILED", { error: error.message });
            }
          }
        }
        return parts.join(" ");
...
export const summarizePdf = async (req, res) => {
  try {
    ...
    try {
      ...
      trackBackendEvent("OPTIONAL_AUTH_FAILED", { error: error.message });
    }
    ...
    (async () => {
      try {
        ...
        trackBackendEvent("USER_STATS_UPDATE_FAILED", { error: error.message });
      }
    })();
...
`;
    ...
    try {
      ...
      return res.status(502).json({
        message: "Summarization failed because the model returned an invalid response.",
        ...(process.env.NODE_ENV !== "production" && { rawOutput: summaryResultText }),
      });
    }
